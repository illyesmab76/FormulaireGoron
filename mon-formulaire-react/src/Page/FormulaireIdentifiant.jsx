...
function FormulaireIdentifiant() {
  ...
  const buildTrigramme = (prenom, nom) => {
  if (!prenom || !nom) return "";

  const p = prenom.trim().toUpperCase();
  const n = nom.trim().toUpperCase();

  if (p.length < 1 || n.length < 2) return "";

  return `A_${p[0]}${n.slice(0, 2)}`;
};
  ...
  setForm((prev) => {
    const updatedForm = {
      ...prev,
      [name]: value,
      ...(name === "nom" && { Monsieur: value }),
    };

    //Si le nom ou le prénom change → on génère le trigramme
    if (name === "nom" || name === "prenom") {
      const trigramme = buildTrigramme(
        name === "prenom" ? value : prev.prenom,
        name === "nom" ? value : prev.nom
      );

      if (trigramme) {
        updatedForm.IdTSE = trigramme;
        updatedForm.IdVPN = trigramme;
        updatedForm.IdPC = trigramme;
      }
    ...
    <Box sx={{ minHeight: "100vh", p: 4 }}>
      ...
      <Box component="form" onSubmit={handleSubmit} sx={{ maxWidth: 1200, mx: "auto" }}>
        ...
        <FormRow
          ...
          input1={<InputMui label="Identifiant" name="IdTSE" value={form.IdTSE} onChange={handleChange} fullWidth />}
